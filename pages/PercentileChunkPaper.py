...
COLLECTION_NAME = "StatChunkPaper"
...
if prompt := st.chat_input("What can I help with?"):
    ...
    if wcd_response.objects:  # Check if the list is not empty
        wcd_obj = wcd_response.objects[0]
        retrived_ans = wcd_obj.properties['chunk']
    else:
        print("No matching objects found in Weaviate.")
        retrived_ans=""
    
    user_prompt = f"""
    Using this data "{retrived_ans}", respond to this prompt: "{prompt}"
    """
